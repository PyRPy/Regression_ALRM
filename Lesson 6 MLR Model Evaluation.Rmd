...
colnames(coolhearts) <- c("Inf1", "Area", "Group", "X2", "X3")
# str(coolhearts)
...
# head(coolhearts)
# tail(coolhearts)
model.1 <- lm( Inf1 ~ Area + X2 + X3, data=coolhearts) # canno have 'inf' ? # changed to Inf1
summary(model.1)
```
...
```{r message=FALSE}
attach(coolhearts)
plot(Area, Inf1, type="n", ylim=c(-0.2, 1),
     xlab="Size of area at risk (grams)",
     ylab="Size of infarcted area (grams)")
for (i in 1:32) points(Area[i], Inf1[i], pch=Group[i], col=Group[i])
...
anova(model.312) # Sequential (type I) SS # default ?
detach(allentest)
```
...
((0.31453+0.01981)/2) / (0.54491/28) # Partial F-stat = 8.589969 # two ways to get same answer
...
# SSE(Area) = 0.87926
0.33434 / 0.87926 # Partial R-squared (X2, X3 | Area) = 0.3802516 # here we care about SSE
